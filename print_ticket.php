...
$id_vente = isset($_POST['id_vente']) ? intval($_POST['id_vente']) : 0;
echo "ID de la vente : " . $id_vente . "<br>";

// Récupérer la manifestation sélectionnée depuis la requête POST
$manifestationSelectionnee = isset($_POST['manifestationSelectionnee']) ? $_POST['manifestationSelectionnee'] : '';
echo "Manifestation sélectionnée : " . $manifestationSelectionnee . "<br>";

// Définir le nom de la manifestation en fonction du choix
switch ($manifestationSelectionnee) {
    case '1ermai-midi':
    case '1ermai-soir':
...
echo "Nom de la manifestation : " . $nom_manifestation . "<br>";

// Requête pour récupérer les produits vendus dans la vente avec leur quantité
$sql = "SELECT Produits.nom_produit, VenteProduits.quantite
        FROM VenteProduits
            ...
            if ($connect === false) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    for ($i = 0; $i < $quantite; $i++) {
                        ...
                        $write_result = socket_write($socket, $escpos_data, strlen($escpos_data));
                        if ($write_result === false) {
                            echo "Erreur lors de l'écriture sur la socket : " . socket_strerror(socket_last_error($socket)) . "<br>";
                        } else {
